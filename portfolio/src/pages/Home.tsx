...
export default function Home() {
  ...
  return (
    <div style={{ backgroundColor: '#111111', color: '#E8E8E8' }} className="min-h-screen page-enter">
      <Navbar />

      {/* Hero */}
      <section className="flex flex-col justify-center min-h-screen px-8 md:px-20 pt-20">
        <p style={{ color: '#06B6D4' }} className="text-sm uppercase tracking-widest mb-4 hero-subtitle">Software Engineer & Game Developer</p>
        <h1 className="text-5xl md:text-7xl font-medium leading-tight mb-6 hero-name" style={{ color: '#E8E8E8' }}>
          Suraj<br />Karthikeyan.
        </h1>
        <p style={{ color: '#A0A0A0' }} className="text-xl max-w-lg leading-relaxed mb-8 hero-body">
          Building immersive experiences and robust systems — from VR medical simulations to audio plugins.
        </p>
        <div className="flex gap-4 hero-btns">
        ...
        <p style={{ color: '#06B6D4' }} className="text-sm mt-16 hero-btns">↓ scroll</p>
      </section>

      {/* About */}
      <div ref={aboutAnim.ref} className={`fade-up ${aboutAnim.isVisible ? 'visible' : ''}`}>
      ...
      {/* Projects */}
      <div ref={projectsAnim.ref} className={`fade-up ${projectsAnim.isVisible ? 'visible' : ''}`}>
          ...
          <div className="mb-16">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {projects.filter(p => p.category === 'game').map(project => (
                <a key={project.id} href={`/projects/${project.id}`} className="project-card group rounded-xl overflow-hidden" style={{ border: '1px solid #2A2A2A' }}>
      ...
      {/* Skills */}
      <div ref={skillsAnim.ref} className={`fade-up ${skillsAnim.isVisible ? 'visible' : ''}`}>
      ...
      {/* Contact */}
      <div ref={contactAnim.ref} className={`fade-up ${contactAnim.isVisible ? 'visible' : ''}`}>
